...
app.post('/api/authors', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const { name } = req.body;
...
app.patch('/api/authors/:id', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
...
app.delete('/api/authors/:id', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
...
// Response: 200 OK + objeto actualizado  |  404  |  400
app.patch('/api/posts/:id', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const id = parseInt(req.params.id);
